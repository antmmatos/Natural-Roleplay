        ...
        if (not uiOpen) then
                    ...
                    if (not tempPos.whitelistJobName or jobName == tempPos.whitelistJobName) then
                        ...
                        if (tempDist <= tempActionDist) then
                            ...
                            if (not IsHudHidden()) then
                                DrawHelpText('Pressione ~' .. Config.Keys.action.name .. '~ para abrir a oficina.', true)
                            end
            ...
            if (customConfigPosIndex) then
                ...
                if (playerVeh == 0 or playerVeh ~= customVehicle) then
                    ...
                    local tempDist = GetDistanceBetweenCoords(playerPos.x, playerPos.y, playerPos.z, tempPos.pos.x,
                    tempPos.pos.y, tempPos.pos.z, true)
                    if (tempDist > tempActionDist) then
                        closeUI(1, 1)
                    end
                end
...
function openUI()
    ...
    if (playerVeh ~= 0 and DoesEntityExist(playerVeh)) then
        ...
        customCamMain = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', camPos.x, camPos.y, camPos.z, -35.0, 0.0,
        headingToObject, GetGameplayCamFov(), false, 2)
        customCamSec = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', camPos.x, camPos.y, camPos.z, -35.0, 0.0,
        headingToObject, GetGameplayCamFov(), false, 2)
