		...
		if job.service ~= nil then
			if GetCurrentPedWeapon(ped,"WEAPON_KNIFE") then
				...
				if animal ~= nil then
					...
					if GetDistanceBetweenCoords(playerpos.x,playerpos.y,playerpos.z,animalpos.x,animalpos.y,animalpos.z,true) <= 1.5 then
						sleep = 1
						if marker_id == nil then TriggerEvent("CORE_MARKERMENU:ShowMarker_c",cfg.job.service.skin_animal_text,function(_id) marker_id = _id end) end
