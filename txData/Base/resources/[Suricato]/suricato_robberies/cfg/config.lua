...
cfg.DebugMode = false -- Ativo apenas para testes

cfg.DisableRevivePlayer = true

cfg.WebhookScreenshot = '' -- WEBHHOK ONDE SERÁ ENVIADO A TELA DO JOGADOR CASO ALGUM ADMINISTRADOR TIRE A PRINT ENQUANTO ESTÁ ESPECTANDO
cfg.WebhookStart = '' -- WEBHOOK PARA NOTIFICAR QUANDO UM ROUBO INICIA (E SUAS INFORMAÇÕES)
cfg.WebhookFinish = '' -- WENHOOK PARA NOTIFICAR QUANDO UM ROUBO FINALIZA (E SEU RALATÓRIO)
cfg.WebhookKill = '' -- WEBHOOK PARA NOTIFICAR AS KILLS DENTRO DOS ROUBOS QUE ESTÃO OCORRENDO
cfg.WebhookCombatLog = '' -- WEBHOOK PARA NOTIFICAR UM POSSÍVEL COMBAT-LOGGING, SÓ AVISARÁ SE ELE DESCONECTAR ENQUANTO O ROUBO NÃO ESTIVER ACABADO
...
cfg.BlipCooldown = 300 -- TEMPO EM SEGUNDOS QUE APARECERÁ O BLIP COM A ROTA PARA A POLÍCIA
cfg.BanditsRange = 75 -- DISTÂNCIA QUE PEGARÁ AS PESSOAS QUANDO O BANDIDO FOR DELEGAR
cfg.PaymentAfterEnd = false -- PAGAR O CARA QUE PUXOU APENAS DEPOIS QUE ACABAR O ROUBO
cfg.MarkerDisappearInRobbery = true -- TRUE PARA DISAPARECER TODOS OS BLIPS ENQUANTO O CARA ESTÁ ROUBANDO
cfg.ComebackPlayerAfterDrop = true -- SE TRUE, IRÁ INSERIR O JOGADOR NOVAMENTE NO ROUBO, SE ELE KITAR E VOLTAR

cfg.ComandoPermissao = 'admin.permissao' -- PERMISSÃO PARA ACESSAR AS INTERFACES ADMIN DO SCRIPT

cfg.PermissaoPolicial = 'policia.permissao' -- PERMISSÃO QUE TODO O POLICIAL APTO À ALGUMA AÇÃO TEM
cfg.UseHierarquia = false -- OPÇÃO QUE IGNORA A HIERARQUIA E MANDA A MENSAGEM PARA TODOS OS POLICIAIS DE UMA VEZ SÓ, O PRIMEIRO QUE ACEITAR TERÁ QUE DELEGAR OU CANCELAR
cfg.HierarquiaPolicial = { -- NOME DO GRUPO, NÃO A PERMISSÃO.
    [1] = 'Capitão',
}

cfg.KillDistance = true -- DESATIVE SE SUA BASE NÃO USA CÓDIGO PARA MATAR A DISTÂNCIA
...
cfg.StaffPermission = 'admin.permissao' -- PERMISSÃO QUE SOMENTE OS STAFFS QUE PODERÃO ACEITAR ROUBOS TEM
...
cfg.Messages = { -- NOTIFICAÇÕES DAS QUAIS É FORNECIDO ALGUNS DADOS PARA A CONFIGURAÇÃO DO SEU JEITO (DEIXAR A STRING VAZIA, CASO NÃO QUEIRA QUE ESSA MENSAGEM APAREÇA)
    ...
    requestNotExist = 'O request já foi aceito, ou excedeu o tempo limite!',
    ...
    notEnoughCops = 'Policiais insuficientes! ({{actual}}/{{required}})', -- variáveis: {{required}} (número necessário de policiais), {{actual}} (número atual de policiais)
