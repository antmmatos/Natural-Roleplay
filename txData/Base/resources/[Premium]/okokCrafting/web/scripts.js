								...
								queueRow += `
									<div class="d-flex justify-content-between w-100 queue-item-concluded mt-2">
										<div class="d-flex align-items-center">
											<img src="icons/${queue[i].item}.png" class="image_components">
											<div class="ms-2">
	...
	for(var i = 0; i < num; i++) {
		for(var i1 = 0; i1 < craft2[i].job.length; i1++){
			if(selectedCategory == "general_cat" || craft2[i].category == selectedCategory){
				if(craft2[i].job[i1] == job2 || craft2[i].job[i1] == ''){
					if(!craftData.UseXP){
...
function claimall(t) {
	if(CraftQueue[0] != undefined && CraftQueue[0].isDone){
...
function updateSideMenu(quantity){
	...
	var recipe = ``;
	var img = `
		<img src="icons/${sideCraftMenu.itemNameID}.png" class="image_itemselected">
	...
	for(var i = 0; i < num; i++){
		...
		if(sideCraftMenu.isDis){
			recipe += `
				<div class="d-flex align-items-center mx-1">
					<img src="icons/${sideCraftMenu.recipe[i][0]}.png" class="image_components">
