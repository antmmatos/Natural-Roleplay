...
RegisterServerEvent('CORE_JOB_LUMBERJACK:CheckIfItemExists_s') -- THIS EVENT CHECKS FOR ITEMS
AddEventHandler('CORE_JOB_LUMBERJACK:CheckIfItemExists_s',function(player,item,itemamount,cb)
	if cfg.framework == "esx" then
	...
	if cfg.framework == "nunoradioman" then
		TriggerEvent("CORE:GetUserID_s",player,function(user_id)
			TriggerEvent("CORE_INVENTORYSYSTEM:GetMaximumInventoryItem_s",user_id,item,function(amount)
				if amount >= itemamount then
					cb(true)
				else
					cb(false)
...
RegisterServerEvent('CORE_JOB_LUMBERJACK:GiveItem_s') -- THIS EVENT ADDS ITEMS
AddEventHandler('CORE_JOB_LUMBERJACK:GiveItem_s',function(player,item,amount)
	if cfg.framework == "esx" then
		local xPlayer = Functions.GetPlayerFromId(player)
		if item == "cash" then
