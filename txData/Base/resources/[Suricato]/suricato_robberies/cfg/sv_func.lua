...
config = {
    ...
    getBackpack = function(user_id)
        if user_id then 
            return vRP.getInventoryMaxWeight(user_id) - vRP.getInventoryWeight(user_id)
        end
    end,
    ...
    giveAwardRobbery = function(user_id, amount, moneytype) -- moneytype: 'darkmoeny' para dinheiro sujo, e 'money' para dinheiro limpo
        if user_id and amount and moneytype then 
            local options = {
                darkmoney = function()
                    config.giveInventoryItem(user_id, 'dinheirosujo', amount)
    ...
    killPlayer = function(source) -- FUNÇÃO QUE SETA A VIDA DO PLAYER
        fClient.killPlayer(source)
    end,
    ...
    warnChat = function(name)
        TriggerClientEvent('chatMessage',-1,"911",{64,64,255},"(10-76) - "..name)
    end 
}
...
RegisterNetEvent('suricato_roubos:enterPlayer', function(source)
    -- evento de entrada
...
RegisterNetEvent('suricato_roubos:leavePlayer', function(source)
    -- evento de saída
