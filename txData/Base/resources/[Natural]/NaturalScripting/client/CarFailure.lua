cfg = {
    ...
    weaponsDamageMultiplier = 2.00,  -- How much damage should the vehicle get from weapons fire. Range 0.0 to 10.0, where 0.0 is no damage and 10.0 is 10x damage. -1 = don't touch
...
function tireBurstLottery()
    local tireBurstNumber = math.random(300)
    if tireBurstNumber == 5 then
...
if cfg.torqueMultiplierEnabled or cfg.preventVehicleFlip or cfg.limpMode then
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(100) -- Alterado de 10 para 50
            if IsPedInAnyVehicle(PlayerPedId(), false) then
                if cfg.sundayDriverthen then
                    local factor = 1.0
        ...
        if isPedDrivingAVehicle() then
            ...
            if pedInSameVehicleLast == true then
                ...
                local newFDeformationDamageMult = fDeformationDamageMult ^
                    cfg
                    .deformationExponent                                                  -- Pull the handling file value closer to 1
                if cfg.deformationMultiplier ~= -1 then
                    SetVehicleHandlingFloat(vehicle, 'CHandlingData',
                        'fDeformationDamageMult', newFDeformationDamageMult * cfg.deformationMultiplier)
                end                                                                       -- Multiply by our factor
                if cfg.weaponsDamageMultiplier ~= -1 then
                    SetVehicleHandlingFloat(vehicle, 'CHandlingData',
                        'fWeaponDamageMult', cfg.weaponsDamageMultiplier / cfg.damageFactorBody)
