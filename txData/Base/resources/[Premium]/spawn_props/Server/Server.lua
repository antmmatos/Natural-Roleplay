RegisterCommand('objects', function(source, args)
    if exports["NaturalScripting"]:isAllowedTo(source, "admin") then
        MySQL.Async.fetchAll('SELECT * FROM props_spawn ORDER BY id DESC',
            {
...
CreateThread(function()
    MySQL.Async.execute("CREATE TABLE IF NOT EXISTS props_spawn( id int AUTO_INCREMENT, prop varchar(200), pos_x varchar(10), pos_y varchar(10), pos_z varchar(10), heading varchar(10), status int DEFAULT 1, folder varchar(50) DEFAULT 'none', PRIMARY KEY(id) )"
        , {}, function() end)
    MySQL.Async.execute("ALTER TABLE props_spawn ADD COLUMN IF NOT EXISTS folder varchar(50) DEFAULT 'none' ", {},
        function() end)
end)
...
AddEventHandler('lg: openFolder', function(data)
    local idJ = source
    MySQL.Async.fetchAll('SELECT * FROM props_spawn WHERE folder = @folder ORDER BY id DESC',
        {
            ['@folder'] = data.folder
        }, function(result)
