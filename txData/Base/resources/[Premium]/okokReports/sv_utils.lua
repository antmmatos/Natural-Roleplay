...
playerReportWebhookLink = 'https://discord.com/api/webhooks/1107323208102846574/YVqEsTYVz50IOaoAzwHyVXTU81VSX7KYaUJZV6jTT6oOiE1SXeAKyTcJcsu9M7Cfs2mK'   -- Player Report webhook

bugReportWebhookLink = 'https://discord.com/api/webhooks/1107323254529605662/gDL3mBLJZe5TFSFo9O12R8tp9qnqb5T-I0GxPZKNJTfN9zPiGKuHAugBfNjsSKIQcKBX'      -- Bug Report webhook

questionReportWebhookLink = 'https://discord.com/api/webhooks/1107323500710084759/lzgckYqXdIHr-QCQTV6VYcv-gk59g5uZK3-v4XXbOy5xlOdTJ9nUbwDwUv9rSJtzg29f' -- Question webhook

adminWebhookLink = 'https://discord.com/api/webhooks/1107324745545945128/iBa1Ub9QBGRub83vCgo4WETLAYM7SlYuV0ZLPyVP4QEq9Dh82ZpleoD9w9a6numV3azx'          -- Admin related activities webhook (bring/go to/answer/conclude)

playerWebhookLink = 'https://discord.com/api/webhooks/1107325626756628520/UfAL5r_Gop7pqDgg_-JKTbKL6tnRvydYGIjc2R91d_Sfh6YpERvpnAFA9PufmbP9jgUl'         -- Player related activities webhook (answer/cancel)
...
AddEventHandler('okokReportsV2:conclude', function(report_id)
    ...
    if admin_identifier ~= nil and Config.SaveRespondedReports then
        MySQLinsert(
        'INSERT INTO okokreports (admin_identifier) VALUES(@admin_identifier) ON DUPLICATE KEY UPDATE admin_identifier=@admin_identifier, responded_reports=responded_reports+1',
        {
    ...
    TriggerClientEvent('okokReportsV2:Notification', _source,
    interp(Config.Notifications['adm_rep_concluded'].text, { id = reportsList[report_id].id }), 'adm_rep_concluded')
    TriggerClientEvent('okokReportsV2:Notification', reportsList[report_id].source,
    interp(Config.Notifications['rep_concluded'].text, { id = reportsList[report_id].id }), 'rep_concluded')
...
AddEventHandler('okokReportsV2:addMyReportAnswer', function(msg, selectedWindow, reportSelected)
    ...
    if selectedWindow == "clientWithReport" then
        ...
        table.insert(reportsList["rep_" .. _source].msgs, '<span class="chat-name">' .. name .. '</span>: ' ..
        msg .. ' <br>')
        ...
        if reportsList["rep_" .. _source].admin_source ~= nil then
            TriggerClientEvent('okokReportsV2:updateAnswers', reportsList["rep_" .. _source].admin_source,
            reportsList["rep_" .. _source].msgs)
            TriggerClientEvent('okokReportsV2:Notification', reportsList["rep_" .. _source].admin_source,
            interp(Config.Notifications['player_answered'].text, { name = name, id = reportsList["rep_" .. _source].id }),
            'player_answered')
        end
        ...
        table.insert(reportsList[reportSelected].msgs,
        '<span class="chat-name being-attended">' .. name .. '</span>: ' .. msg .. ' <br>')
        ...
        TriggerClientEvent('okokReportsV2:updateAnswers', reportsList[reportSelected].source,
        reportsList[reportSelected].msgs)
        TriggerClientEvent('okokReportsV2:Notification', reportsList[reportSelected].source,
        Config.Notifications['adm_answered'].text, 'adm_answered')
        if adminWebhookLink ~= '' then
            local identifierlist = ExtractIdentifiers(_source)
            local webhookData = {
                color = Config.adminWebhookColor,
...
function discordWebhook(data, url)
    ...
    end, 'POST', json.encode({ username = Config.BotName, embeds = information }),
