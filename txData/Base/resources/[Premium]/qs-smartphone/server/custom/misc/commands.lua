        ...
        if Config.Framework == 'esx' then
            for k, v in pairs(ESX.GetPlayers()) do
                ...
                if mailData.button == nil then
                    MySQL.Async.execute("INSERT INTO `player_mails` (`identifier`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES ('"..Player.identifier.."', '"..mailData.sender:gsub("'", "´").."', '"..mailData.subject:gsub("'", "´").."', '"..table.concat(mailData.message, ' '):gsub("'", "´").."', '"..GenerateMailId().."', '0')", {})
                else
                    MySQL.Async.execute("INSERT INTO `player_mails` (`identifier`, `sender`, `subject`, `message`, `mailid`, `read`, `button`) VALUES ('"..Player.identifier.."', '"..mailData.sender:gsub("'", "´").."', '"..mailData.subject:gsub("'", "´").."', '"..table.concat(mailData.message, ' '):gsub("'", "´").."', '"..GenerateMailId().."', '0', '"..json.encode(mailData.button).."')", {})
                end
