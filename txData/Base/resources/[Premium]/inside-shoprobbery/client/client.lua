local PlayerData = {}
local started, blockstart, CashRegister, restart, firstlock, secondlock = false, false, false, false, false, false
		...
		if not blockstart then
			if(GetDistanceBetweenCoords(coords,Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z, true) < 9.0) and not started then
				...
				if(GetDistanceBetweenCoords(coords,Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z, true) < 1.5) then	
					...
					DrawText3Ds(Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z+1.0, 'Para iniciar um assalto, pressione [~g~E~w~]')
					if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
						ESX.TriggerServerCallback('inside-shoprobbery:SecBetwNextRob', function(YouCan)	
							if YouCan then
								ESX.ShowNotification("Coleta informações com o informador sobre a loja a ser assaltada.")
								...
								ESX.ShowNotification("Alguém fez a minha tarefa recentemente, espere um pouco")
							end
						end)
					end
				else
			...
			if(GetDistanceBetweenCoords(coords,Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z, true) < 9.0) then	
				...
				if(GetDistanceBetweenCoords(coords,Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z, true) < 1.5) then	
					...
					if started then
						DrawText3Ds(Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z+1.0, 'Para cancelar, pressione [~r~E~w~]')
						if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
							DeletePed(CodeNPC)
						...
						DrawText3Ds(Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z+1.0, 'Alguém está me comissionando no momento')		
					end			
				else
	...
	while true do
		if started then
			...
			if started and not CashRegister then
				ESX.ShowNotification("O tempo para completar a tarefa acabou")
				DeletePed(CodeNPC)
				pedwalking = false
				RemoveBlip(ShopBlip)
				DeletePed(ShopNPC)
	...
	while true do
		...
		if started then
			...
			if(GetDistanceBetweenCoords(coords,NPCcoords.x, NPCcoords.y, NPCcoords.z, true) < 1.5) then	
				sleep = 5
				DrawText3Ds(NPCcoords.x, NPCcoords.y, NPCcoords.z+1.0, 'Para tirar os documentos, pressione [~r~E~w~]')
				if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
					pedwalking = false
					ClearPedTasks(CodeNPC)
					...
					ESX.ShowNotification("O assalto é na loja "..Shop.name)
					ShopRobbery()
				end
			end
		end	
...
AddEventHandler('inside-shoprobbery:PoliceNotifyC', function(coords)
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
		...
		ESX.ShowNotification("Está a ocorrer um assalto à loja marcada no mapa.")
	...
	while true do
		...
		local coords = GetEntityCoords(ped)
			if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].PedPos.x, Config.Shops[Shop.name].PedPos.y, Config.Shops[Shop.name].PedPos.z, true) < 6.5) then	
				...
				DrawText3Ds(Config.Shops[Shop.name].PedPos.x, Config.Shops[Shop.name].PedPos.y, Config.Shops[Shop.name].PedPos.z+1.0, 'Olá, em que posso ajudar?')
				if IsPlayerFreeAiming(PlayerId()) then
					...
					if IsPedFleeing(targetPed) and targetPed == ShopNPC then						
						...
						while displaying do
							...
							DrawText3Ds(coordsPed['x'], coordsPed['y'], coordsPed['z'] + 1.2, "Já estou a abrir!")
						end
						atlocation = false
						Citizen.CreateThread(function()
							Wait(10000)
						...
						Citizen.CreateThread(function()
							...
							while true do
								...
								if Config.Shops[Shop.name].Time <= 0 then
									ESX.ShowNotification("O dinheiro está pronto para ser retirado")
									ClearPedTasks(ShopNPC)
						...
						while true do
							...
							if Config.Shops[Shop.name].Time > 0 then
								if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].PedWalks.x, Config.Shops[Shop.name].PedWalks.y, Config.Shops[Shop.name].PedWalks.z, true) < 3.5) then
									DrawText3Ds(Config.Shops[Shop.name].PedWalks.x, Config.Shops[Shop.name].PedWalks.y, Config.Shops[Shop.name].PedWalks.z+1.0, 'Tempo: '..tostring(Config.Shops[Shop.name].Time))
								end
								...
								if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].PedWalks.x, Config.Shops[Shop.name].PedWalks.y, Config.Shops[Shop.name].PedWalks.z, true) < 1.5) then	
									DrawText3Ds(Config.Shops[Shop.name].PedWalks.x, Config.Shops[Shop.name].PedWalks.y, Config.Shops[Shop.name].PedWalks.z + 1, "Para tirar o dinheiro, pressione [~r~E~w~]")
									if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
										FreezeEntityPosition(ShopNPC, false)
										...
										ESX.TriggerServerCallback('inside-shoprobbery:payout', function(money)
											ESX.ShowNotification("Roubaste €"..money.."")
		...
		if CashRegister then
			...
			if Config.Shops[Shop.name].CashRegister1 then 
				if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].CashRegister1.x, Config.Shops[Shop.name].CashRegister1.y, Config.Shops[Shop.name].CashRegister1.z, true) < 1.5) and not Config.Shops[Shop.name].CashRegister1.robbed then	
					...
					ShowFloatingHelpNotification("~INPUT_PICKUP~ - Para roubar a caixa registadora", vector3(Config.Shops[Shop.name].CashRegister1.x, Config.Shops[Shop.name].CashRegister1.y, Config.Shops[Shop.name].CashRegister1.z))
			...
			if Config.Shops[Shop.name].CashRegister2 then
				...
				ShowFloatingHelpNotification("~INPUT_PICKUP~ - Para roubar a caixa registadora", vector3(Config.Shops[Shop.name].CashRegister2.x, Config.Shops[Shop.name].CashRegister2.y, Config.Shops[Shop.name].CashRegister2.z))
					if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped2, false) then
			...
			if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].ShopPos.x, Config.Shops[Shop.name].ShopPos.y, Config.Shops[Shop.name].ShopPos.z, true) > 15.0) and (started or cassettedone < 2) then
				...
				CashRegister = false
				TriggerServerEvent('inside-shoprobbery:playerdroppedStop')
				ESX.ShowNotification("Afastaste-te da zona")
				RemoveBlip(ShopBlip)
				DeletePed(ShopNPC)
				if Config.Shops[Shop.name].CashRegister1 then
					Config.Shops[Shop.name].CashRegister1.robbed = false
