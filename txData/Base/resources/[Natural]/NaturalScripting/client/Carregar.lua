	...
	if not carry.InProgress then
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerServerEvent("Natural:Carregar:Server:Sync",targetSrc)
				ensureAnimDict(carry.personCarrying.animDict)
				carry.type = "carrying"
			else
				drawNativeNotification("Não há jogadores por perto")
			end
		else
			drawNativeNotification("Não há jogadores por perto")
		end
		...
		TriggerServerEvent("Natural:Carregar:Server:Stop",carry.targetSrc)
		carry.targetSrc = 0
	end
end,false)

RegisterNetEvent("Natural:Carregar:Client:Sync")
AddEventHandler("Natural:Carregar:Client:Sync", function(targetSrc)
...
RegisterNetEvent("Natural:Carregar:Client:Stop")
AddEventHandler("Natural:Carregar:Client:Stop", function()
	carry.InProgress = false
	ClearPedSecondaryTask(PlayerPedId())
	DetachEntity(PlayerPedId(), true, false)
end)
