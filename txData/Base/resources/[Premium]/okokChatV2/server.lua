...
local Webhook = 'https://discord.com/api/webhooks/1107311983281315860/b8nimI10SM4RjO3Roq5isfVet9ZbMwW5Jx3YwKozyqXQ14vdzG5pTWlStQ7v1LXvmtmR' -- PUT YOUR WEBHOOK LINK HERE
		...
		if chatsHidden[source] == nil then
			...
			if Config.ShowIDOnMessage then
				showToClosePlayersAdmins(xPlayer, function(players)
					TriggerClientEvent('chat:addMessage', players, {
						template = '<div class="chat-message me"><i class="'..Config.MeIcon..'"></i> <b><span style="color: #79b8fa">['..Config.MeMessageTitle..'] {0}</span>&nbsp;<span class="time">{2}</span></span></b><div class="message">{1}</div></div>',
	...
	RegisterCommand(Config.DoCommand, function(source, args, rawCommand)
		...
		if chatsHidden[source] == nil then
			...
			if Config.ShowIDOnMessage then
				showToClosePlayersAdmins(xPlayer, function(players)
					TriggerClientEvent('chat:addMessage', players, {
						template = '<div class="chat-message do"><i class="'..Config.DoIcon..'"></i> <b><span style="color: #79faa4">['..Config.DoMessageTitle..'] {0}</span>&nbsp;<span class="time">{2}</span></span></b><div class="message">{1}</div></div>',
						args = { playerNameAdmin, message, time }
					})
				end)
			end
	...
	RegisterCommand(Config.TryCommand, function(source, args, rawCommand)
		...
		if chatsHidden[source] == nil then
			...
			if Config.ShowIDOnMessage then
				showToClosePlayersAdmins(xPlayer, function(players)
					TriggerClientEvent('chat:addMessage', players, {
						template = '<div class="chat-message try"><i class="'..Config.TryIcon..'"></i> <b><span style="color: #f34971">['..Config.TryMessageTitle..'] {0}</span>&nbsp;<span class="time">{2}</span></span></b><div class="message">{1}</div></div>',
						args = { playerNameAdmin, message, time }
					})
				end)
			end
...
if Config.EnableStaffCommand then
	RegisterCommand(Config.StaffCommand, function(source, args, rawCommand)
		...
		if Config.StaffSteamName then
			playerName = getSteamName(xPlayer.identifier)
		end
...
if Config.EnableServerAnnouncement then
    RegisterCommand(Config.ServerAnnouncementCommand, function(source, args, rawCommand)
        ...
        if source ~= 0 and source ~= -1 then
            xPlayer = ESX.GetPlayerFromId(source) -- or xPlayer = QBCore.Functions.GetPlayer(source) for QB
        end
...
if Config.EnableAdvertisementCommand then
	RegisterCommand(Config.AdvertisementCommand, function(source, args, rawCommand)
		...
		local time = os.date(Config.DateFormat)
		local playerName = Config.ShowIDOnMessageForEveryone and getName(xPlayer.identifier).." ["..source.."]" or getName(xPlayer.identifier)
		local playerNameAdmins = getName(xPlayer.identifier).." ["..source.."]"
...
if Config.EnablePM then
	RegisterCommand(Config.PMCommand, function(source, args, rawCommand)
		...
		local playerName = Config.ShowIDOnMessageForEveryone and getName(xPlayer.identifier).." ["..source.."]" or getName(xPlayer.identifier).." ["..source.."]"
		...
		xTargetName = getName(xTarget.identifier)
	...
	function OOC(source, message)
		...
		if chatsHidden[source] == nil then
			...
			if Config.ShowIDOnMessage then
				showToClosePlayersAdmins(xPlayer, function(players)
					TriggerClientEvent('chat:addMessage', players, {
						template = '<div class="chat-message ooc"><i class="'..Config.OOCIcon..'"></i> <b><span style="color: #ababab">['..Config.OOCMessageTitle..'] {0}</span>&nbsp;<span class="time">{2}</span></span></b><div class="message">{1}</div></div>',
						args = { playerNameAdmin, message, time }
					})
				end)
			end
				...
				if not isAnonymousChatBlacklisted(xPlayer) then
					if anonymousList[source] == nil then
						if bankMoney >= Config.AnonymousPrice then
							xPlayer.removeAccountMoney('bank', Config.AnonymousPrice)
...
function showOnlyForAnonymous(anonymous)
	for k,v in ipairs(onlinePlayers) do
		...
		if xPlayer ~= nil then
			if not Config.ShowIDOnMessage and chatsHidden[xPlayer.source] == nil and not isAnonymousChatBlacklisted(xPlayer) then
				anonymous(v)
			elseif Config.ShowIDOnMessage and chatsHidden[xPlayer.source] == nil and not isAdmin(xPlayer) and not isAnonymousChatBlacklisted(xPlayer) then
				anonymous(v)
			end
		end
	end
...
function showOnlyForAdminsAnonymous(admins)
	for k,v in ipairs(onlinePlayers) do
		local xPlayer = ESX.GetPlayerFromId(v)
		if xPlayer ~= nil then
			if isAdmin(xPlayer) and chatsHidden[xPlayer.source] == nil and not isAnonymousChatBlacklisted(xPlayer) then
				admins(v)
			end
		end
	end
...
function showOnlyForAdmins(admins)
	for k,v in ipairs(onlinePlayers) do
		local xPlayer = ESX.GetPlayerFromId(v)
		if xPlayer ~= nil then
			if isAdmin(xPlayer) and chatsHidden[xPlayer.source] == nil then
				admins(v)
			end
		end
	end
...
function showAll(players)
	for k,v in ipairs(onlinePlayers) do
		local xPlayer = ESX.GetPlayerFromId(v)
		if xPlayer ~= nil then
			if not Config.ShowIDOnMessage and chatsHidden[xPlayer.source] == nil then
				players(v)
			elseif Config.ShowIDOnMessage and chatsHidden[xPlayer.source] == nil and not isAdmin(xPlayer) then
				players(v)
			end
		end
	end
...
function showMuteMessageToEveryone(id, players)
	for k,v in ipairs(onlinePlayers) do
		local xPlayer = ESX.GetPlayerFromId(v)
		if xPlayer ~= nil then
			if chatsHidden[xPlayer.source] == nil and id ~= v then
				players(v)
			end
		end
	end
...
function showForJob(job, players)
	for k,v in ipairs(onlinePlayers) do
		local xPlayer = ESX.GetPlayerFromId(v)
		if xPlayer ~= nil then
			if not Config.ShowIDOnMessage and xPlayer.job.name == job and chatsHidden[xPlayer.source] == nil then
				players(v)
			elseif Config.ShowIDOnMessage and not isAdmin(xPlayer) and xPlayer.job.name == job and chatsHidden[xPlayer.source] == nil then
				players(v)
			end
		end
	end
...
function showForJobAdmins(job, players)
	for k,v in ipairs(onlinePlayers) do
		local xPlayer = ESX.GetPlayerFromId(v)
		if xPlayer ~= nil then
			if xPlayer.job.name == job and chatsHidden[xPlayer.source] == nil and isAdmin(xPlayer) then
				players(v)
			end
		end
	end
...
function getSteamName(identifier)
	local name = nil
	MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', {
		['@identifier'] = identifier
	}, function(db_name)
