...
function Lang(what)
    if not Locale[what] then return "TRANSLATION NOT FOUND" end -- If we cant find a translation, use the english one.
    return Locale[what]
end

ClothingConfig.Commands = {
    [Lang("TOP")] = {
...
local Bags = {
    -- This is where bags/parachutes that should have the bag sprite, instead of the parachute sprite.
    [40] = true,
    [41] = true,
    [44] = true,
    [45] = true
...
ClothingConfig.ExtraCommands = {
    ...
    [Lang("RESET")] = {
        Func = function() ResetClothing(true) end,
        Sprite = "reset",
        Desc = Lang("Reset2"),
        Name = Lang("Reset"),
        OffsetX = 0.12,
...
function ToggleClothing(which, extra)
    ...
    local Ped = PlayerPedId()
    local Cur = {
        -- Lets check what we are currently wearing.
    ...
    if which ~= "Mask" then
        if not Gender then return false end -- We cancel the command here if the person is not using a multiplayer model.
    end
...
function ToggleProps(which)
    ...
    local Ped = PlayerPedId()
    local Cur = {
        -- Lets get out currently equipped prop.
        Id = Prop.Prop,
        Ped = Ped,
    ...
    if not Prop.Variants then
        ...
        local Gender = IsMpPed(Ped)
        if not Gender then return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
...
for k, v in pairs(ClothingConfig.Commands) do
    RegisterCommand(k, v.Func)
    TriggerEvent("chat:addSuggestion", "/" .. k, v.Desc)
end
if ClothingConfig.ExtrasEnabled then
    for k, v in pairs(ClothingConfig.ExtraCommands) do
        RegisterCommand(k, v.Func)
        TriggerEvent("chat:addSuggestion", "/" .. k, v.Desc)
    end
end
