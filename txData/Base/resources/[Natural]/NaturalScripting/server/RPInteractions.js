...
RegisterCommand('do', (source, args) => {
	...
	if (!blacklisted(message)) {
		// make sure they're not on cooldown (they could try spamming the command)
		if (!tryCooldown[source] || tryCooldown[source] < GetGameTimer()) {

			tryCooldown[source] = GetConvarInt('3dme_cooldown', 1500) + GetGameTimer()
...
let tryCooldown = {}
RegisterCommand('tentar', (source, args) => {
	let message = `${args.join(' ')}`
	...
	if (!blacklisted(message)) {
		...
		if (!tryCooldown[source] || tryCooldown[source] < GetGameTimer()) {
			if (getRandomArbitrary(0, 100) > 50) {
				message = message + ": ~g~sucesso";
			} else {
				message = message + ": ~r~falhado";
			}
			tryCooldown[source] = GetConvarInt('3dme_cooldown', 1500) + GetGameTimer()
...
on('playerDropped', () => {
	...
	doCooldown[source] = null
	tryCooldown[source] = null
})