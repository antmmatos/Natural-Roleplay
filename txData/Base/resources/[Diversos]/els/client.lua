	...
	while true do
		...
		if IsPedInAnyVehicle(playerped, false) then
			...
			if GetPedInVehicleSeat(veh, -1) == playerped then
				...
				if state_indic[veh] ~= ind_state_o and state_indic[veh] ~= ind_state_l and state_indic[veh] ~= ind_state_r and
					state_indic[veh] ~= ind_state_h then
				...
				--- IS ANY LAND VEHICLE ---
				if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and
