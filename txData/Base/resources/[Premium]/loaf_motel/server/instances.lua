        ...
        for instance, data in pairs(Instances) do
            if data then
                for i, guest in pairs(data.users) do
                    if guest == src then
                        return TriggerClientEvent("esx:showNotification", Strings["already_inside"], "warning")
                    end
                end
            end
        end
            ...
            if keys then
                ...
                if not has_key then
                    if GlobalState[instance_name] then
                        ...
                        return TriggerClientEvent("esx:showNotification", src, Strings["no_key"], "warning")
                    end
                else
                    should_enter = true
                end
                ...
                if should_enter then
                            ...
                            if result and result[1] then
                                ...
                                if spawn then
                                    ...
                                    if Config.Inventory == "ox" then
                                        exports.ox_inventory:RegisterStash(instance_name, Strings["inventory"], 50, 25000, result[1].identifier)
                                        --print(instance_name,result[1].identifier)
                                    end
                    ...
                    TriggerClientEvent("esx:showNotification", src, Strings["door_locked"], "warning")
                end
            end
        end)
    end)
