function OpenBossMenu(society, close, options)
	options = options or {}
	local elements = {
		{unselectable = true, icon = "fas fa-user", title = "Menu Chefe"}
	}
	...
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local elements = {
				...
				{icon = "fas fa-wallet", title = "Amount", input = true, inputType = "number", inputPlaceholder = "Amount to change grade salary..", inputMin = 1, inputMax = ConfigSociety.MaxSalary, name = "gradesalary"},
				{icon = "fas fa-check", title = "Confirm", value = "confirm"}
			}
			...
			if element.value == "confirm" then
				...
				if amount == nil then
					...
					OpenManageSalaryMenu(society, options)
				elseif amount > ConfigSociety.MaxSalary then
