...
AddEventHandler('esx_society:checkSocietyBalance', function(society)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		TriggerClientEvent("esx:showNotification", xPlayer.source, TranslateCap('check_balance', ESX.Math.GroupDigits(account.money)), "info")
	end)
end)
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	...
	for i=1, #(xPlayers) do 
		...
		if ConfigSociety.EnableESXIdentity and name == GetPlayerName(xPlayer.source) then
	...
	if ConfigSociety.EnableESXIdentity then
...
ESX.RegisterServerCallback('esx_society:setJobSalary', function(source, cb, job, grade, salary)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' then
		if salary <= ConfigSociety.MaxSalary then
