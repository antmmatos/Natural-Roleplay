...
AddEventHandler("qs-smartphone:valetCarSetOutside", function(a)
    ...
    elseif Config.Framework == 'qb' then
        local b = source;
        MySQL.Async.execute("UPDATE " .. vehiclesTable .. " SET `state` = @state WHERE `plate` = @plate", {["@plate"] = a, ["@state"] = 0})
    end
end)
...
AddEventHandler("qs-smartphone:getInfoPlate", function(plate)
    ...
    elseif Config.Framework == 'qb' then
        ...
        local veh_datastore = MySQL.Sync.fetchAll("SELECT mods, vehicle, plate FROM " .. vehiclesTable .. " WHERE `citizenid` ='"..player.citizenid.."' AND `plate` ='"..plate.."' ", {})
        if veh_datastore and veh_datastore[1] then
