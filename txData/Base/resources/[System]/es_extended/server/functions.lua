...
function Core.SavePlayer(xPlayer, cb)
    MySQL.prepare(
        'UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ? WHERE `identifier` = ?',
        {
            ...
            function(results)
                if results then
                    if type(cb) == 'function' then
                        ...
                        print(('[^2INFO^7] Saved %s %s over %s ms'):format(count,
                            count > 1 and 'players' or 'player', (os.time() - time) / 1000000))
                    end
                end
            end)
